...
void displayMenu(Peer &peer, std::string &log)
{
    ...
    if (ImGui::BeginPopupModal("Set Sync Period", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::Button("Set"))
        {
            ...
            {
                timer.start(1000*60*60, std::bind(&Peer::syncDataSources, &peer));
            }
            else if (peer.Pgetter() == MINUTELY)
            {
                timer.start(1000*60, std::bind(&Peer::syncDataSources, &peer));
            }
            else if (peer.Pgetter() ==QUATERLY)
            {
                timer.start(1000*60*15, std::bind(&Peer::syncDataSources, &peer));
            }
            else if (peer.Pgetter() == DAILY)
            {
                timer.start(1000*60*60*24, std::bind(&Peer::syncDataSources, &peer));
            }
            else if (peer.Pgetter() == WEEKLY)
            {
                timer.start(1000*60*60*24*7, std::bind(&Peer::syncDataSources, &peer));
            }
            else if (peer.Pgetter() == MONTHLY){
                timer.start(long long(1000*60*60*24*30, std::bind(&Peer::syncDataSources, &peer));
            }
