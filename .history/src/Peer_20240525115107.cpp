...
void Peer::handleClientConnection(SOCKET clientSocket)
{
    ...
    if (received > 0)
    {
        ...
        {
            // 构建文件路径
            std::string filePath = "E:/Project/DistributedProject/file/img/" + path;
...
void Peer::downloadDataSource(const std::string &peerAddress, const DataSource &dataSource)
{
    ...
    {
        // 构建保存文件的路径
        std::string filePath = "E:/Project/DistributedProject/file/image/" + dataSource.name;
...
String Peer::displayNodeInfo()
{
    ...
    String res = R("
        Known peers:
            ") +
                  String(knownPeers.size()) + R("
        Available data sources:
            ") +
                  String(availableDataSources.size());
        Available data sources:
