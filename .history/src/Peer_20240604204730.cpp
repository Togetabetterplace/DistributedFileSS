...
/**
 * Finds and returns the string representation of the synchronization period.
 *
 * @param key The integer representation of the synchronization period.
 *
 * @return A string representing the synchronization period.
 *
 * @note The function uses a series of if-else statements to match the input key
 *       with the corresponding synchronization period and return the corresponding string.
 *
 * @warning The function does not handle any invalid input keys.
 *          If an invalid key is provided, the behavior of the function is undefined.
 */
std::string find_Period(int key){
    if(key==0){
        return "Immediately\n";
    }else if(key==1){
        return "Minutely\n";
