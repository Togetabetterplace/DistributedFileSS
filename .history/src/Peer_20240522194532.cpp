...
/**
 * @brief Adds a data source to the list of available data sources.
 *
 * This function adds a new data source to the list of available data sources.
 * It is thread-safe and ensures that the data source is added only once.
 *
...
void Peer::addDataSource(const DataSource &dataSource)
{
    ...
    availableDataSources.push_back(dataSource); ///< Add the data source to the list.
}
...
/**
 * @brief Synchronizes the data sources with known peers.
 *
 * This function retrieves the list of available data sources and known peers,
 * then enqueues a download task for each data source from each known peer.
 * The tasks are executed concurrently using the thread pool.
 *
 * @return void
 */
...
//     {
//         std::lock_guard<std::mutex> lock(mtx); ///< Lock the mutex to ensure thread safety.
//         dataSources = availableDataSources; ///< Copy the available data sources to a local vector.
//     }

//     /**
//      * @brief Enqueues a download task for each data source from each known peer.
//      *
//      * This lambda function is used to enqueue a download task for each data source
//      * from each known peer. The task is executed concurrently using the thread pool.
//      *
...
void Peer::syncDataSources()
{
    ...
    for (const DataSource &dataSource : dataSources)
    {
        for (const std::string &peerAddress : knownPeers)
        {
            // 确保 lambda 正确捕获了 dataSource 和 peerAddress
            threadPool.enqueue([this, &peerAddress, &dataSource]() mutable
                               { downloadDataSource(peerAddress, dataSource); });
        }
    }
}
...
void Peer::handleClientConnection(SOCKET clientSocket)
{
    ...
    if (received > 0)
    {
        ...
        if (method == "GET")
        {
            std::string filePath = "E :\\Project\\DistributedProject\\flie\\img "+ path; // !?
...
void Peer:: downloadDataSource(const std::string &peerAddress, const DataSource &dataSource)
{
    ...
    if (received > 0)
    {
        ...
        std::string filePath = "E:\\Project\\DistributedProject\\flie1\\image\\" + dataSource.name;
