...
int main(int, char **)
{
    ...
    {
        timer.start(60000, peer.syncDataSources());
    }
    else if(peer.SyncPeriod == DAILY){
        timer.start(86400000, peer.syncDataSources());
    }else{
        // timer.start(1000, peer.syncDataSources());
        flag = 1;
    }
