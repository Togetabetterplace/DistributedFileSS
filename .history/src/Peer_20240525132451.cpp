...
void Peer::handleClientConnection(SOCKET clientSocket)
{
    ...
    if (received > 0)
    {
        ...
        {
            // 构建文件路径
            std::string filePath = "E:/Project/Distributed/DistributedFileSS/file/img/" + path;
...
void Peer::downloadDataSource(const std::string &peerAddress, const DataSource &dataSource)
{
    ...
    {
        // 构建保存文件的路径
        std::string filePath = "E:/Project/Distributed/DistributedFileSS/test_down_file/image/" + dataSource.name;
